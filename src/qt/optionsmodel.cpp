...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizecafeAmount"))
        settings.setValue("nAnonymizecafeAmount", 1000);

    nAnonymizeCafeAmount = settings.value("nAnonymizecafeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizecafeAmount"))
        SoftSetArg("-anonymizecafeamount", settings.value("nAnonymizecafeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cafe.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizecafeAmount:
            return QVariant(nAnonymizeCafeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizecafeAmount:
            nAnonymizeCafeAmount = value.toInt();
            settings.setValue("nAnonymizeCafeAmount", nAnonymizeCafeAmount);
            emit anonymizecafeAmountChanged(nAnonymizeCafeAmount);
            break;
        case CoinControlFeatures:
